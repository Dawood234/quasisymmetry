#!/usr/bin/env bash
#SBATCH --job-name=h2o_631g_parent
...
#SBATCH --output=/scratch/%u/las_h2o_631g_parent_%j.out
#SBATCH --export=ALL
...
if [[ ! -f "${PROJECT_DIR}/run_h2o_631g_parent_reference.py" ]]; then
...
RUN_DIR="${LAS_RUN_DIR:-${SCRATCH_ROOT}/alris/quasisymmetry/h2o/6-31g/parent_reference/job_${SLURM_JOB_ID}}"
...
THREADS="${H2O_PARENT_THREADS:-${SLURM_CPUS_PER_TASK}}"
MODE="${H2O_PARENT_MODE:-production}"
BOND_DIMS="${H2O_PARENT_BOND_DIMS:-100,200,350,500,750}"
STAGE_SWEEPS="${H2O_PARENT_STAGE_SWEEPS:-4,4,6,6,8}"
REVERSE_DIMS="${H2O_PARENT_REVERSE_DIMS:-750,750,500,500,350,350,250,250}"
SYMMETRY_MODE="${H2O_PARENT_SYMMETRY:-su2}"
ORDERING="${H2O_PARENT_ORDERING:-fiedler}"
STACK_MEM_GB="${H2O_PARENT_STACK_MEM_GB:-8}"
...
command=(
    srun python -u run_h2o_631g_parent_reference.py
    --run_dir "${RUN_DIR}"
...
if [[ "${H2O_PARENT_FCI_VALIDATION:-0}" == "1" ]]; then
    command+=(--fci_validation)
fi
command+=("$@")

echo "Restart command:"
echo "LAS_PROJECT_DIR=${PROJECT_DIR} LAS_VENV=${LAS_VENV} LAS_RUN_DIR=${RUN_DIR} sbatch ${PROJECT_DIR}/submit_h2o_631g_parent_reference.sh --resume"
echo
"${command[@]}"
